...
class SpikeDrivenTransformer(nn.Module):
    ...
    def forward_features(self, x, hook=None):
        ...
        i = 0
        for blk in block:
            x, _, hook, qk_temp, kv_mp_temp, qk_hadmard_product_sum_temp = blk(x, hook=hook)
            qk.append(qk_temp)
            kv_mp.append(kv_mp_temp)
            qk_hadmard_product_sum.append(qk_hadmard_product_sum_temp)

        x = x.flatten(3).mean(3)
        return x, hook, qk, kv_mp, qk_hadmard_product_sum
    ...
    def forward(self, x, hook=None):
        ...
        x, hook, qk, kv_mp, qk_hadmard_product_sum = self.forward_features(x, hook=hook)
        x = self.head_lif(x)
        if hook is not None:
