...
class MS_Attention_RepConv_qkv_id(nn.Module):
    ...
    def forward(self, x):
        ...
        qk_hadmard_product = q.mul(k)
        qk_hadmard_product_sum = qk_hadmard_product.sum(dim=-1, keepdim=False)
        qk_matmul = torch.matmul(q, k.transpose(-1, -2))
        qk = qk_matmul / math.sqrt(8)
        qk = qk.sum(dim=-2, keepdim=False)
        x = k.transpose(-2, -1) @ v
        kv = x
        x = (q @ x) * self.scale
        ...
        attn_lif_mp = self.attn_lif.v_seq
        x = x.reshape(T, B, C, H, W)
        x = x.flatten(0, 1)
        x = self.proj_conv(x).reshape(T, B, C, H, W)

        return x, qk, attn_lif_mp, qk_hadmard_product_sum
...
class MS_Block(nn.Module):
    ...
    def forward(self, x):
        attn_x, qk, attn_lif_mp, qk_hadmard_product_sum = self.attn(x)
        x = x + attn_x
        x = x + self.mlp(x)
...
class Spiking_vit_MetaFormer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.block3:
            x, qk_item, attn_lif_mp_item, qk_hadmard_product_sum_item = blk(x)
            qk.append(qk_item)
            attn_lif_mp_list.append(attn_lif_mp_item)
            qk_hadmard_product_sum.append(qk_hadmard_product_sum_item)
        ...
        for blk in self.block4:
            ...
            attn_lif_mp_list.append(attn_lif_mp_item)
            qk_hadmard_product_sum.append(qk_hadmard_product_sum_item)
        return x, qk, attn_lif_mp_list, qk_hadmard_product_sum  # T,B,C,N

    def forward(self, x):
        x = (x.unsqueeze(0)).repeat(self.T, 1, 1, 1, 1)
        x, qk, attn_lif_mp, qk_hadmard_product_sum = self.forward_features(x)
