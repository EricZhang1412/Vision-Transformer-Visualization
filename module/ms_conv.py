...
class MS_SSA_Conv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        # qk_matmul = qk_matmul.reshape(T, B, 8, int(math.sqrt(N)), int(math.sqrt(N)))
        ...
        # qk_hadmard_product_sum = qk_hadmard_product
        kv = self.talking_heads_lif(kv)
        kv_mp = self.talking_heads_lif.v_seq
        # qk = self.talking_heads_lif2(qk)
        if hook is not None:
        ...
        x = x + identity
        return x, v, hook, qk, kv_mp, qk_hadmard_product_sum
...
class MS_Block_Conv(nn.Module): 
    ...
    def forward(self, x, hook=None):
        x_attn, attn, hook, qk, kv_mp, qk_hadmard_product_sum = self.attn(x, hook=hook)
        x, hook = self.mlp(x_attn, hook=hook)
        return x, attn, hook, qk, kv_mp, qk_hadmard_product_sum
